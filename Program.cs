...
void Program()  // N 5
{
    while (true)
    {
        System.Console.Write(" Введите номер задачи(34, 36, 38 или 1 для выхода): ");
        ...
        switch (task)
        {
            ...
            case 36:
                Console.Clear();
                System.Console.WriteLine(" Задача 36: Задайте одномерный массив, заполненный случайными числами. Найдите сумму элементов, стоящих на нечётных позициях."); // очень длинная строка
...
int[] RandomArray(int size, int minValue, int maxValue)  // Заполнения массива случайными числами
{
...
int CountPositiveNumbers(int[] Array)  // Подсчет положительных элементов
{
    int count = 0;
    foreach (var value in Array)
    {
        value % 2 == 0 ? count == 0 ; // я не понимаю почему тут ошибка
    }
    return count;
}

int SumOfNegatives(int[] Array)  // Подсчет элементов с нечетными индексами
{
    int sum = 0;
...
double[] RandomDoubleArray(int size, int minValue, int maxValue)  // Заполнения массива случайными числами
{
...
double MinMax(double[] Array)  // Нахождения разницы между максимальным и минимальным значениями в массиве
